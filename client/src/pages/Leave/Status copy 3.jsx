...
function LeaveStatus() {
  ...
  const columns = [
    ...
    {
      field: 'to_date', headerName: 'End Dates', flex: 1,
      renderCell: (params) => {
    ...
    {
      field: 'requested_date', headerName: 'Requested Date', flex: 1,
      renderCell: (params) => {
        const date = params.row?.requested_date;
        return (<span>{params.row?.requested_date ? new Date(params.row.requested_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(' ', '-') : 'N/A'}</span>);
      },
    },
    {
    ...
    { field: 'reject_reason', headerName: 'Reject Reason', flex: 1 },
    {
      ...
      renderCell: (params) => (
        <img
          src={`http://localhost:3500${params.row.profile_picture}`}
          alt="Profile"
    ...
    <div className="max-w-4xl mx-auto p-4">
      ...
      {userData ? (
        <div className="text-sm text-gray-700 mb-4">
          ...
          <p className="inline-block bg-green-100 px-2 py-1 m-2 rounded-md">User ID: {userData.userid}</p>
          <p className="inline-block bg-green-100 px-2 py-1 m-2 rounded-md">Role: {userData.role_lpm}</p>
        </div>
      ) : (
        <span className="text-md bg-success-light rounded text-success px-1 ml-0 ltr:ml-2 rtl:ml-2">
  {"Sample User, Admin"}
