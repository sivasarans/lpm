...
const LeaveForm = () => {
  ...
  const handleApply = async () => {
    ...
    formData.append("user_name", "Siva Saran");
    ...
    try {
      ...
      if (applyLeave.fulfilled.match(resultAction)) {
        showToast('Leave applied and balance updated successfully!', 'success');
        handleReset();
      } else {
...
{userData ? (
  <div className="text-sm text-gray-700 mb-4">  {/* Increased mt-40 to mt-60 for more space */}
    ...
    <p className="inline-block px-2 py-1 m-2 rounded-md">User ID: {userData.user_id}</p>
  </div>
) : 
...
>
  {[
    ...
    <option key={leave.short} value={leave.short}>
      {leave.full} {/* Full name is shown to the user */}
    </option>
  ))}
</select>
  ...
  <div className="grid grid-cols-1 gap-4 items-center mt-4">
      <div>
        ...
        <input
          ...
          className="form-input w-full p-2 rounded-md cursor-pointer"
        ...
        {showPicker && (
          <div className="mt-2 z-50">
            <DateRange
              ...
              onChange={(item) => (
                setRange([item.selection]), setShowPicker(false)
